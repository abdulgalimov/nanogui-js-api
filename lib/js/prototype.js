...
NanoguiModule.postRun.push(function() {
  ...
  Nanogui.ComboBox.prototype.addChange = buttonsPrototype.addChange;
  Nanogui.ComboBox.prototype._setCallbacks = buttonsPrototype._setCallbacks;
  Nanogui.Slider.prototype.addChange = buttonsPrototype.addChange;
  Nanogui.Slider.prototype._setCallbacks = buttonsPrototype._setCallbacks;
  //
  ...
  //
  Nanogui.createVectorString = function() {
    var vector = new Nanogui.VectorString();
    for (var i=0; i<arguments.length; i++) {
      vector.push_back(''+arguments[i]);
    }
    return vector;
  };
  //
  //
  function setDefaultValues(className, importantCount, defValues) {
    var originClass = Nanogui[className];
    var newClass = Nanogui[className] = function () {
      if (arguments.length < importantCount) {
        console.error('Конструктор класса Nanogui.'+className+' должен получить минимум параметров: '+importantCount+', получено: '+arguments.length);
      }
      for (var i=0; i<defValues.length; i++) {
        if (arguments.length <= importantCount+i) {
          arguments[importantCount + i] = arguments[importantCount + i] || defValues[i];
        }
      }
      arguments.length = importantCount+defValues.length;
      originClass.apply(this, arguments);
    };
    newClass.prototype = originClass.prototype;
    newClass.prototype.constructor = newClass;
  }
  // setDefaultValues('GroupLayout', 0, [15, 6, 14, 20]);
  setDefaultValues('Label', 2, ['sans-bold', 16]);
  // setDefaultValues('Button', 2, [0]);
